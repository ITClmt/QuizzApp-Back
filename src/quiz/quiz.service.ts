...
export class QuizService {
  ...
  async getQuestions(lang: string = 'en'): Promise<SanitizedQuestion[]> {
    ...
    // if (existing.length < this.QUESTIONS_PER_GAME) {
      const fetchedQuestions = await this.fetchFromOtd(this.QUESTIONS_PER_GAME)
  ...
  private async fetchFromOtd(amount: number) {
  ...
  async validateAnswer(questionId: string, answerIndex: number): Promise<boolean> {
